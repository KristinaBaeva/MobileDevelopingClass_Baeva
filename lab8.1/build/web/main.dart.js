...
define('zapp_user_main', ['dart_sdk', 'flutter_sdk'], (function load__zapp_user_main(dart_sdk, flutter_sdk) {
  ...
  main.MyApp = class MyApp extends framework.StatelessWidget {
    build(context) {
      return new app.MaterialApp.new({home: new scaffold.Scaffold.new({appBar: new app_bar.AppBar.new({title: new text.Text.new("Фотогалерея")}), body: new (T.FutureBuilderOfListOfPhoto()).new({future: main.fetchPhotos(), builder: dart.fn((context, snapshot) => {
              if (snapshot.hasError) {
                return new basic.Center.new({child: new text.Text.new("Ошибка загрузки")});
              } else if (snapshot.hasData) {
                return new scroll_view.GridView.builder({gridDelegate: new sliver_grid.SliverGridDelegateWithFixedCrossAxisCount.new({crossAxisCount: 2}), itemCount: dart.nullCheck(snapshot.data)[$length], itemBuilder: dart.fn((context, index) => new image.Image.network(dart.nullCheck(snapshot.data)[$_get](index).thumbnailUrl), T.BuildContextAndintToImage())});
              } else {
  ...
  main.fetchPhotos = function fetchPhotos() {
    return async.async(T.ListOfPhoto(), function* fetchPhotos() {
      let response = (yield http.get(core.Uri.parse("https://jsonplaceholder.typicode.com/photos")));
      if (response.statusCode === 200) {
        let jsonList = core.List.as(convert.json.decode(response.body));
        return jsonList[$map](main.Photo, dart.fn(json => main.Photo.fromJson(T.MapOfString$dynamic().as(json)), T.dynamicToPhoto()))[$toList]();
      } else {
        dart.throw(core.Exception.new("Ошибка загрузки данных"));
      }
    });
  };
